...
def generate_dataset(params):
    ...
    try:
        ...
        if not os.path.exists(workingDir + "\\workspace"):
            os.makedirs(workingDir + "\\workspace")

        if not os.path.exists(workingDir + "\\outdir"):
                os.mkdir(workingDir + "\\outdir")
        ...
        for fileIndex in range(startIndex, endIndex + 1):
            ...
            copy(in_file_path, workingDir + "\\workspace")

            # Use joern to create AST, CFG, PDG.
            os.chdir(workingDir)
            os.system("..\\joern-cli\\bin\\joern-parse workspace")
            os.system("..\\joern-cli\\bin\\joern-export --repr ast --out outdir\\ast")
            os.system("..\\joern-cli\\bin\\joern-export --repr cfg --out outdir\\cfg")
            os.system("..\\joern-cli\\bin\\joern-export --repr pdg --out outdir\\pdg")
            os.chdir("..")
            ...
            if not ast_paths:
                ...
                for folder in os.listdir(workingDir + "\\outdir"):
            ...
            cfg_paths = extract_cfg_paths(os.path.relpath(workingDir + "\\outdir\\cfg"))
            pdg_paths = extract_pdg_paths(os.path.relpath(workingDir + "\\outdir\\pdg"))
