...
def store_tokens_paths(token_dict, path_dict):
    with FileLock("path_ids.txt.lock"):
        with open("path_ids.txt", 'a', encoding="utf-8") as f:
            for path, path_id in path_dict.items():
                f.write(str(path_id) + '\t' + path + '\n')

    with FileLock("terminal_ids.txt.lock"):
        with open("terminal_ids.txt", 'a', encoding="utf-8") as f:
